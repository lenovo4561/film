<template>
    <div id="cinema-detail">
      ...
      <div class="movie-info" v-for="(item,index) in hasMovieInfo" :key="index" v-show="movieIndex===Number(index)">
        ...
        <span class="main"><span class="name">{{item.name}}</span><span class="score"><span class="num" v-if="item.score && item.score > 0">{{Number(item.score).toFixed(1)}}分</span><span v-else style="font-size: .28rem">暂无评分</span></span></span>
        <span class="intro">
