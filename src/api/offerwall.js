...
service.interceptors.request.use(
  config => {
    ...
    console.log(`${config.method.toUpperCase()} 请求参数（含签名）:`, config.method === 'get' ? config.params : config.data)
...
export function getUserCoins() {
  ...
  if (!userId) {
    // 尝试从 cookie 中读取
    console.log('[getUserCoins] localStorage 中无 userId，尝试从 cookie 读取...')
  ...
  return axios.get('/api/getUserCoins', {
    ...
    console.error('[getUserCoins] 错误响应:', error.response && error.response.data)
    throw error
  })
}
