              ...
              <div class="item" v-for="(item,index) in hotMovieList.slice(0, 6)" :key="index">
                ...
                <div style="position: relative">
                  <div class="describe">
                    ...
                    <span class="score" v-if="item.score && item.score > 0"><i class="interger">{{Number(item.score).toFixed(1).split('.')[0]}}</i>.<i class="fraction">{{Number(item.score).toFixed(1).split('.')[1]}}</i></span>
                  </div>
                </div>
      ...
      methods:{
        ...
        async loadMovieList(){
          try {
            ...
            console.log(`获取到 ${json.data.length} 部电影`);
            ...
            console.log(`热映电影: ${this.hotMovieList.length} 部`);
            console.log(`即将上映: ${this.notShowMovieList.length} 部`);
